...
select case when (select auto_trade=false and human_execution_only=true from fwios.dashboard_refresh_payload_v1()->'data'->'current_action'->0 as x(auto_trade boolean,human_execution_only boolean)) then 'PASS' else 'FAIL' end as execution_isolation;
